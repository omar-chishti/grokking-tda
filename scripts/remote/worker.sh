...
run_one() {
    ...
    # A filename is capped at 255 bytes; a long override line blows past that and the
    # redirect fails with no diagnostic, so the run dies looking like a training error.
    local tag
    if [[ "${overrides}" =~ run_name=([^[:space:]]+) ]]; then
        tag=${BASH_REMATCH[1]}
    else
