...
scheduler. They are kept for the central-HPC path.
...
here runs non-interactively, and a *locked* key fails exactly like a rejected one:
...
The checkout, the environment and the results tree all live on `/vol/bitbucket`, which every DoC
machine mounts, so they are built once and every GPU host sees them; home directories have a
few-gigabyte quota and a CUDA venv alone exceeds it. `sync` pushes the source, installs
`uv` into `~/.local/bin` if it is missing, and runs `uv sync --frozen` on an idle GPU host — the
shell server forbids heavy processes — resolving the CUDA 12.1 torch wheels pinned for Linux in
`pyproject.toml`.
...
exactly as it does for a local run.

## Etiquette

These are shared desktops that students sit at.

- `probe` already skips a machine whose free GPU memory is under `MIN_FREE_MB`, whose load average
  is over `MAX_LOAD`, or which is running one of our own workers. Do not widen those filters to
  take a busy machine. (Logged-in-user count is not among them: these hosts carry dozens of
  detached SSH sessions, so it says nothing about whether anyone is working.)
- Keep `JOBS_PER_HOST` modest, so a workstation someone is sitting at is never saturated.
...
| `MIN_FREE_MB` | `6000` | skip a GPU with less free memory |
| `MAX_LOAD` | `2.0` | skip a machine whose CPU is already busy |
| `REMOTE_BASE` | `/vol/bitbucket/$REMOTE_USER/grokking-tda` | checkout, environment and results |
...
per second in aggregate. On a single lab GPU the same run is expected to take a few minutes, and a
full programme to finish in a few hours of wall time across a handful of hosts.

Check it on the first sweep: `status` reports steps per second from each run's `events.jsonl`.
